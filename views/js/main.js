...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    var element = document.getElementsByClassName("randomPizzaContainer");
    var times = element.length;
    for (var i = 0; i < times; i++) {
      element[i].style.width = dx + '%';
    }
  }
...
function updatePositions() {
  ...
  for (var j = 0; j < 5; j++) {
    phases[j] = Math.sin(scroll + j);
    //    console.log(j + " - "+ phases[j]);
  }
  for (var i = 0; i < itemsLength; i++) {
    //items[i].style.transform = 'translate(' + phase + 'px, 0,0)';
    item = items[i];
    item.style.left = item.basicLeft + 100 * phases[i % 5] + 'px';
  }
//  ticking = false;
